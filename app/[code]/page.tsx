...
// Detects if the request is coming from a known bot (social media crawlers, SEO bots, etc.)
...
// Checks if the target URL is an image file
function isLikelyImage(url: string) {
  return /(\.png|\.jpg|\.jpeg|\.gif|\.webp|\.bmp)(\?.*)?$/i.test(url);
}

// Fetches metadata (title, description, image) from the target URL for social previews
async function fetchMetaFromTarget(targetUrl: string) {
  try {
    // Skip HTML parsing if target is an image
    ...
    // Fetch the page HTML with a short timeout to avoid slow URLs
    ...
    // Fallback metadata if target URL fails to load
...
export async function generateMetadata(props: any) {
  ...
  // Determine base URL from env or headers (needed for OG/Twitter metadata)
  ...
  // Prepare metadata for social platforms
  const title = preview.title
    ? `TinyLink – ${preview.title}`
    : `TinyLink – ${link.code}`;
...
export default async function RedirectPage(props: any) {
  ...
  // Retrieve the short link from the database
  const rows = await db`SELECT * FROM links WHERE code = ${code}`;
  if (rows.length === 0) redirect("/");

  const link = rows[0];

  // Show preview only for bots or when ?preview=1 is used
  const showPreview = isBot(ua) || sp.preview === "1" || sp.preview === 1;

  // Normal user → increase click count and redirect instantly
  if (!showPreview) {
    await db`
      UPDATE links 
  ...
  // Show preview UI (rich preview for social media apps)
  ...
  return (
    <div className="max-w-2xl mx-auto p-8">
      ...
      <div className="card">
        {meta.image && (
          // Image preview for OG/Twitter rich cards
          <img
            src={meta.image}
            alt="Preview"
        ...
        <p className="text-gray-700 mt-2">
          This short link provides shareable preview metadata for social apps
          and chats. Click below to continue.
        </p>

        <a href={link.target_url} className="btn-primary mt-4 inline-flex">
          Continue to destination
        </a>
      </div>

      <p className="text-sm text-gray-500 mt-4">
        Add <code>?preview=1</code> to force preview:{" "}
        <code>/{code}?preview=1</code>
      </p>
    </div>
  );
}
